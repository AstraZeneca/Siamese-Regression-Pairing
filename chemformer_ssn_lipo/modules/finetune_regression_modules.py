...
# from modules.debug.tokeniser import MolEncTokeniser
# from modules.debug.data.datasets import ReactionDataset 
# from modules.debug.data.datamodules import _AbsDataModule
# from modules.debug.models.pre_train import _AbsTransformerModel
...
class RegPropDataset(ReactionDataset):
    ...
    Args: 
        data_path (str): the path of a csv file, 
            which must contain the columns [ID1, ID2, comp1, comp2, prop, SET]
    """
    def __init__(self, df):

        data_in_1 = df["comp1"].tolist()
        data_in_2 = df["comp2"].tolist()
        data_out = df["prop"].tolist()    

        super().__init__(data_in_1, data_in_2,data_out)
...
class RegPropDataModule(_AbsDataModule):
    ...
    def _collate(self, batch, train=True):
        ...
        SMILES_tokens_1 = token_output["SMILES_tokens_1"]
        SMILES_mask_1 = token_output["SMILES_mask_1"]
        SMILES_tokens_2 = token_output["SMILES_tokens_2"]
        SMILES_mask_2 = token_output["SMILES_mask_2"]
        targets = token_output["props"]
        def output(SMILES_tokens, SMILES_mask):
            ...
            return SMILES_token_ids, SMILES_pad_mask
        SMILES_token_ids_1, SMILES_pad_mask_1 = output(SMILES_tokens_1, SMILES_mask_1)
        SMILES_token_ids_2, SMILES_pad_mask_2 = output(SMILES_tokens_2, SMILES_mask_2)
        collate_output = {
            #stay consistent with Ross code for the dictionary keys
            "encoder_input_1": SMILES_token_ids_1,
            "encoder_pad_mask_1": SMILES_pad_mask_1,
            "encoder_input_2": SMILES_token_ids_2,
            "encoder_pad_mask_2": SMILES_pad_mask_2,
            "target": targets
        }
    ...
    def _prepare_tokens(self, batch):
        ...
        comp1, comp2, props= tuple(zip(*batch))
        ...
        def prepare(inpSMILES):
            ...
            return SMILES_tokens, SMILES_mask

        SMILES_tokens_1, SMILES_mask_1 = prepare(comp1)
        SMILES_tokens_2, SMILES_mask_2 = prepare(comp2)


        token_output = {
            "SMILES_tokens_1": SMILES_tokens_1,
            "SMILES_mask_1": SMILES_mask_1,
            "SMILES_tokens_2": SMILES_tokens_2,
            "SMILES_mask_2": SMILES_mask_2,
...
class RegrTransformerModel(pl.LightningModule):    
    ...
    def __init__(
        ...
        # Regression part
        self.mlp = nn.Sequential(nn.Linear(self.d_model, 64),nn.ReLU(),nn.Linear(64, 1, bias = False))
        self.loss_fn = nn.MSELoss()
        self._init_params()
    def out(self, encoder_input, encoder_pad_mask):
...
#         memory = torch.mean(memory,dim=0)  #memory.reshape(memory.shape[1], -1) #to take the average from all last outputs of encoder
    ...
    def forward(self, x):
        ...
        """
        # compound 1
        encoder_input_1 = x["encoder_input_1"]
        encoder_pad_mask_1 = x["encoder_pad_mask_1"].transpose(0, 1)
        # compound 2
        encoder_input_2 = x["encoder_input_2"]
        encoder_pad_mask_2 = x["encoder_pad_mask_2"].transpose(0, 1)

        out1 = self.out( encoder_input_1, encoder_pad_mask_1)
        out2 = self.out( encoder_input_2, encoder_pad_mask_2)
        x = out1 - out2
        x = self.mlp(x)
...
class FineTuneTransformerModel(pl.LightningModule):
    ...
    def __init__(
        ...
        self.hidden_fc = nn.Linear(self.d_premodel, 1024)
        self.drp = nn.Dropout(dropout_p)        
        self.predict_fc = nn.Linear(1024, 1)
        self.loss_fn = nn.MSELoss()
    ...
    def forward(self, x):
        ...
        out1, out2 = self.premodel(x)
        out1 = out1[1,:,:] #in the 2rd element is the gene_symbol i.e. 1nd in python 
                                         # eg '^','<OPRD1>','O','=','C','1','N',...
...
class EncoderOfBARTModel(_AbsTransformerModel):
    ...
    def output(self, encoder_input, encoder_pad_mask):
        encoder_embs = self._construct_input(encoder_input)
        memory = self.encoder(encoder_embs, src_key_padding_mask=None)
        return memory
    ...
    def forward(self, x):
        ...
        encoder_input_2 = x["encoder_input_2"]
        encoder_pad_mask_2 = x["encoder_pad_mask_2"].transpose(0, 1)
        out1 = self.output(encoder_input_1, encoder_pad_mask_1)
        out2 = self.output(encoder_input_2, encoder_pad_mask_2)
