...
def make_plot(sim, sd, plot_path, sim_sd):
    #plt.errorbar(sd, sim, yerr=sim_sd, ls = '', capsize = 3)
    plt.scatter(sd, sim)
    plt.xlabel('Uncertainty', fontsize=15)
    plt.ylabel('Average Similarity', fontsize=15)
    plt.tight_layout()
    plt.savefig(f'{plot_path}uncertainty_sim.png')
    plt.clf()
...
if __name__ == "__main__":
    ...
    #smake_plot(df['mean_sim'], df['std_sim'], outpath_mean,df['sd_sim_full'] )
    make_plot(df[f'top{top}_mean'], df[f'top{top}_std'], outpath_top, df['sd_sim_top'] )
